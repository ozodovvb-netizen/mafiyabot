...
@router.message(Command("msend"), F.chat.type.in_({"group", "supergroup"}))
async def cmd_msend(message: Message, command: CommandObject):
...
async def _gift_reply(message: Message, command: CommandObject, diamonds: bool):
    ...
    cmd_name = "gsend" if diamonds else "msend"
